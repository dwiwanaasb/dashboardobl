...
if ($level == 'mitra') {
    if ($witel == '' and $tahun == '' and $pelanggan == '' and $segmen == '' and $status == '') {
        $query = query("SELECT * FROM report_obl WHERE UPPER(proses) = 'MITRA OBL' AND 
    ...
    if ($witel == '' and $tahun == '' and $mitra == '' and $pelanggan == '' and $segmen == '' and $status == '') {
        $query = query("SELECT * FROM report_obl WHERE UPPER(proses) = 'MITRA OBL' 
            ORDER BY folder ASC");
    } else {
        $query = query("SELECT * FROM report_obl WHERE UPPER(proses) = 'MITRA OBL' AND 
            (UPPER(witel) LIKE '%$witel%' AND
...
<head>
    ...
    <script src="js/ajax/ajaxMitraOBL.js"></script>
    <title>Dokumen di Mitra OBL</title>
</head>
...
<body>
    <div class="container">
        ...
        <div class="header">
            ...
            <div class="_2">
                <a href="rankMitraOBL.php"><button style="background-color: #6610F2;" class="rank">Perangkingan</button></a>
                <a href="downloadData/downloadMitraOBL.php?witel=<?= $witel; ?>&tahun=<?= $tahun; ?>&mitra=<?= $mitra; ?>&pelanggan=<?= $pelanggan; ?>&segmen=<?= $segmen; ?>&status=<?= $status; ?>" id="btnDownload"><button class="download" style="background-color: #FD7E14;"><img src="./img/download.png" alt="">Download</button></a>
