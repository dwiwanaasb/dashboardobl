...
if ($level == 'mitra') {
    if ($witel == '' and $tahun == '' and $pelanggan == '' and $segmen == '' and $status == '') {
        $query = query("SELECT * FROM report_obl WHERE UPPER(proses) = 'CANCEL' AND 
    ...
    if ($witel == '' and $tahun == '' and $mitra == '' and $pelanggan == '' and $segmen == '' and $status == '') {
        $query = query("SELECT * FROM report_obl WHERE UPPER(proses) = 'CANCEL' 
            ORDER BY folder ASC");
    } else {
        $query = query("SELECT * FROM report_obl WHERE UPPER(proses) = 'CANCEL' AND 
            (UPPER(witel) LIKE '%$witel%' AND
...
<head>
    ...
    <script src="js/ajax/ajaxCancel.js"></script>
    <title>Cancel</title>
</head>
...
<body>
    <div class="container">
        ...
        <div class="header">
            ...
            <div class="_2">
                <a href="downloadData/downloadCancel.php?witel=<?= $witel; ?>&tahun=<?= $tahun; ?>&mitra=<?= $mitra; ?>&pelanggan=<?= $pelanggan; ?>&segmen=<?= $segmen; ?>&status=<?= $status; ?>" id="btnDownload"><button class="download" style="background-color: #FD7E14;"><img src="./img/download.png" alt="">Download</button></a>
