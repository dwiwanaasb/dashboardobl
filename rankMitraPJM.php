...
<head>
    ...
    <script src="js/ajax/ajaxRankMitraPJM.js"></script>
    <title>Rank Mitra PJM</title>
</head>

<body>
    <div class="container">
        <h1>Rank Mitra PJM</h1>
        ...
        <?php if (isset($_GET["rankBy"])) { ?>
            ...
            $rankVal = $_GET["rankBy"];
            if ($rankVal == 'nilaiKB') {
                $total = query("SELECT nama_vendor, COUNT(*) AS totalOrder, SUM(nilai_kb) AS totalNilaiKB FROM report_obl WHERE UPPER(proses) = 'MITRA PJM' 
                ...
                <table>
                    ...
                    <?php foreach ($total as $row) { ?>
                        <tr>
                            ...
                            $cek_data = mysqli_query($conn, "SELECT nama_vendor FROM rankmitrapjm WHERE nama_vendor = '$namaVendor'");

                            if (mysqli_fetch_assoc($cek_data)) {
                                $query = "UPDATE rankmitrapjm SET nama_vendor = '$namaVendor', total_order = $totalOrder, total_nilaiKB = $totalNilaiKB WHERE nama_vendor = '$namaVendor' ";
                            } else {
                                $query = "INSERT INTO rankmitrapjm (nama_vendor, total_order, total_nilaiKB) VALUES ('$namaVendor', $totalOrder, $totalNilaiKB)";
                            }
                            ...
                            <td class="aksi" id="aksiTd">
                                <div>
                                    <a href="detailRankMitraPJM.php?mitra=<?= $row["nama_vendor"]; ?>"><button class="view"><img src="img/view.png" alt=""></button></a>
                                </div>
                            </td>
                            <?php
                    ...
                    <tr class="sum">
                        <?php
                        $query = query("SELECT SUM(total_order) AS totalOrder, SUM(total_nilaiKB) AS totalNilaiKB FROM rankmitrapjm")[0];
                        ?>
