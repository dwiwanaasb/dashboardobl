...
if ($level == 'mitra') {
    if ($witel == '' and $tahun == '' and $mitra != '' and $pelanggan == '' and $segmen == '' and $status == '') {
        $query = query("SELECT * FROM report_obl WHERE UPPER(nama_vendor) = '$mitra'
                ORDER BY folder ASC");
    } else {
        $query = query("SELECT * FROM report_obl WHERE UPPER(nama_vendor) = '$mitra' AND 
    ...
    if ($witel != '' and $tahun == '' and $mitra == '' and $pelanggan == '' and $segmen == '' and $status == '') {
        $query = query("SELECT * FROM report_obl WHERE UPPER(witel) = '$witel'
                ORDER BY folder ASC");
    } else {
        $query = query("SELECT * FROM report_obl WHERE UPPER(witel) = '$witel' AND 
                (UPPER(segmen) LIKE '%$segmen%' AND
                SUBSTRING(folder, 1, 2) LIKE '%$tahun%' AND 
...
<head>
    ...
    <script src="js/ajax/ajaxTotal.js"></script>
...
<body>
    <div class="container">
        ...
        <div class="header">
            ...
            <div class="_2">
                <a href="downloadData/downloadTotal.php?witel=<?= $witel; ?>&tahun=<?= $tahun; ?>&mitra=<?= $mitra; ?>&pelanggan=<?= $pelanggan; ?>&segmen=<?= $segmen; ?>&status=<?= $status; ?>" id="btnDownload"><button class="download" style="background-color: #FD7E14;"><img src="./img/download.png" alt="">Download</button></a>
