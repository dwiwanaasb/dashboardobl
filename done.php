...
if (isset($_GET["witel"]) or isset($_GET["tahun"]) or isset($_GET["mitra"]) or isset($_GET["pelanggan"]) or isset($_GET["segmen"])) {
    $witel = $_GET["witel"];
    $tahun = $_GET["tahun"];
    $mitra = $_GET["mitra"];
...
if ($level == 'mitra') {
    if ($witel == '' and $tahun == '' and $pelanggan == '' and $segmen == '' and $status == '') {
        $query = query("SELECT * FROM report_obl WHERE UPPER(proses) = 'DONE' AND 
...
<head>
    ...
    <script src="js/ajax/ajaxDone.js"></script>
    <title>Dokumen Done</title>
</head>
    ...
    <div class="container">
        ...
        <div class="header">
            <div class="_1">
                ...
                <input type="text" name="keyword" id="keyword" placeholder="Masukkan keyword..." autocomplete="off" id="keyword">
            </div>
            <div class="_2">
                <a href="downloadData/downloadDone.php?witel=<?= $witel; ?>&tahun=<?= $tahun; ?>&mitra=<?= $mitra; ?>&pelanggan=<?= $pelanggan; ?>&segmen=<?= $segmen; ?>&status=<?= $status; ?>" id="btnDownload"><button class="download" style="background-color: #FD7E14;"><img src="./img/download.png" alt="">Download</button></a>
