...
if ($level == 'mitra') {
    ...
    if ($witel == '' and $tahun == '' and $mitra == '' and $pelanggan == '' and $segmen == '' and $status == '') {
        $query = mysqli_query($conn, "SELECT * FROM report_obl WHERE UPPER(proses) = 'WITEL'
                    ORDER BY folder ASC");
    } else {
        $query = mysqli_query($conn, "SELECT * FROM report_obl WHERE UPPER(proses) = 'WITEL' AND
...
$date = date("d-m-Y");
$fileName = "WITEL-report-obl(" . $date . ")";
